...
/**
 * GLM 配置加载:全部示例共用,约定与 agentscope-test / langgraph4j-test / langchain4j-test 完全一致。
 *
 * 【知识点】
 * 1. 智谱 GLM 开放平台提供 OpenAI 兼容协议,所以 Spring AI 侧不需要任何智谱专用 SDK,
 *    直接用 spring-ai-starter-model-openai(见 GlmModels 和 Web 示例),换 DeepSeek/Kimi/本地 vLLM
 ...
 *    Web 示例走 Spring 自动配置,application.yml 里用 ${glm.xxx} 占位符把 glm.* 映射到
...
public final class GlmConfig {
    ...
    /** 默认向量模型(RAG 示例用) */
    public static final String DEFAULT_EMBEDDING_MODEL = "embedding-3";
    ...
    public static String embeddingModel() {
        return value("embedding-model", "GLM_EMBEDDING_MODEL", DEFAULT_EMBEDDING_MODEL);
    }
