...
```bash
mysql -u root -p -e "CREATE DATABASE IF NOT EXISTS ai_knowledge_base_test DEFAULT CHARACTER SET utf8mb4;"
```
...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/ai_knowledge_base_test?...
    username: root
...
```bash
cd ai-knowledge-base-test/backend
mvn spring-boot:run
```
...
```bash
cd ai-knowledge-base-test/frontend
npm install
npm run dev
```
